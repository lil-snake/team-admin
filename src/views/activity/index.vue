  ...
  <el-main>
    ...
    >
      ...
      <el-form-item label="类型">
        <el-select v-model="searchParm.type" clearable placeholder="请选择类型">
          <el-option
      ...
      <el-form-item>
        ...
        <el-button plain type="danger" icon="Close" @click="resetBtn"
          >重置</el-button
        >
    ...
    <el-table :height="tableHeight" :data="tableList" border stripe>
      ...
      <el-table-column prop="hasJoin" label="可以报名">
        <template #default="scope">
          <div v-if="scope.row.type == '0'">
            <el-tag type="danger" v-if="scope.row.hasJoin == '0'"
              >不可以</el-tag
            >
            <el-tag type="success" v-else>可以</el-tag>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" width="400" label="操作">
        <template #default="scope">
          ...
          >
          <el-button
            v-if="scope.row.type == '0' && scope.row.status == '0'"
            icon="Edit"
            type="warning"
            size="default"
...
const { addBtn, editBtn, deleteBtn, addActivityRef, applyBtn, returnBtn } =
  useActivity(getList);
